...
void registerFrontendRoutes(crow::SimpleApp& app) {
  ...
  ([addSecurityHeaders](const crow::request& req) {
    ...
    if (!checkRateLimit(clientIp)) {logRequest("GET", "/", 429); return crow::response(429);}
  ...
  ([addSecurityHeaders](const crow::request& req) {
    const std::string clientIp = resolveClientIp(req.get_header_value("x-forwarded-for"), req.remote_ip_address);
    if (!checkRateLimit(clientIp)) {logRequest("GET", "/features", 429); return crow::response(429);}
  ...
  ([addSecurityHeaders](const crow::request& req) {
    const std::string clientIp = resolveClientIp(req.get_header_value("x-forwarded-for"), req.remote_ip_address);
    if (!checkRateLimit(clientIp)) {logRequest("GET", "/honours-project", 429); return crow::response(429);}
  ...
  ([](const crow::request& req, const std::string& path) {
    const std::string clientIp = resolveClientIp(req.get_header_value("x-forwarded-for"), req.remote_ip_address);
    if (!checkRateLimit(clientIp)) {logRequest("GET", "/" + path, 429); return crow::response(429);}
