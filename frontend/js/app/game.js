...
function getTextColumns(){const lines=cleanedCSV.trim().split('\n');const headers=parseCSVLine(lines[0]);if(currentColumnTypes&&currentColumnTypes.length>0){const textCols=[];for(let i=0;i<currentColumnTypes.length;i++){const t=currentColumnTypes[i].type;if(t==='NAME'||t==='GENERIC_TEXT'||t==='FREE_TEXT'){if(i<headers.length)textCols.push(headers[i]);}}if(textCols.length>0)return textCols;}
// fallback: old heuristic
const textCols=[];for(let col=0;col<headers.length;col++){let isText=false;for(let row=1;row<Math.min(6,lines.length);row++){const cells=parseCSVLine(lines[row]);const cell=cells[col];if(cell&&isNaN(cell)){isText=true;break;}}if(isText)textCols.push(headers[col]);}return textCols;}
...
async function showMergeSummary(){document.getElementById('mergeGameSection').style.display='none';document.getElementById('completeSection').style.display='block';const summaryDiv=document.createElement('div');summaryDiv.style.cssText='padding:20px;background:#f0f0f0;border-radius:4px;margin-bottom:20px;';summaryDiv.innerHTML='<strong>merge complete:</strong> '+mergeState.mergedCount+' merge groups applied across '+mergeState.allColumns.length+' columns<br/><em style="font-size:12px;">running final deduplication...</em>';document.getElementById('gameBoard').appendChild(summaryDiv);await removeDuplicatesAfterMerge();const lines=cleanedCSV.trim().split('\n');const headers=parseCSVLine(lines[0]);updateTableDisplay(headers);displayAuditLog();autoDownloadCSV();}

async function removeDuplicatesAfterMerge(){if(!checkOnlineAndNotify())return;try{const payload={csvData:cleanedCSV};if(currentColumnTypes&&currentColumnTypes.length>0){payload.columnTypes=currentColumnTypes;}const res=await fetch(window.location.origin+'/api/post-merge-dedup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!res.ok){console.error('Post-merge dedup failed with status',res.status);return;}const data=await res.json();if(data.csvData){cleanedCSV=data.csvData;}if(data.auditLog&&Array.isArray(data.auditLog)){auditOperations.push(...data.auditLog);}console.log('Post-merge dedup: removed '+data.rowsRemoved+' rows');}catch(e){console.error('Post-merge dedup error:',e);}}
